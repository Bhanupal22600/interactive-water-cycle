...
function getHandPositions() {
    ...
    for (let pose of poses) {
        ...
        // Get left wrist (keypoint 15)
        // if (pose.keypoints[15].confidence > 0.7) {
        //     let x = width - pose.keypoints[15].x; // Mirror X coordinate
        //     let y = pose.keypoints[15].y;
...
// Update handleSunInteraction and handleClouds to use mirrored positions
...
function handleClouds() {
    ...
    for (let cloud of clouds) {
        ...
        if (!dragged && mouseIsPressed && dist(mouseX, mouseY, cloud.x, cloud.y) < 50) {
            cloud.x = mouseX;
            cloud.y = mouseY;
        }
...
function handleEvaporation() {
    if (sun.y > height - 300 && !cloudFormed) {
      ...
      if (!vapour.isPlaying()) {
        ...
        // Stop sound when evaporation stops
        if (vapour.isPlaying()) {
            vapour.stop();
        }
    }
